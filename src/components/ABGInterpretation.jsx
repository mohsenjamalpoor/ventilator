...
// کامپوننت نمایش محدوده نرمال برای کودکان
const NormalRangeIndicator = ({ value, normalMin, normalMax, unit, ageGroup = "کودک" }) => {
  const numValue = parseFloat(value);
  if (!value) return null;
...
const ABGInterpretation = ({
  ...
  const validateABG = () => {
    ...
    if (!HCO3) {
      ...
      const HCO3Num = parseFloat(HCO3);
      if (HCO3Num < 5 || HCO3Num > 50) {
        errors.HCO3 = "مقدار HCO3 باید بین 5 تا 50 mEq/L باشد";
        isValid = false;
      } else if (HCO3Num < 22 || HCO3Num > 26) {
        errors.HCO3 = "مقدار HCO3 خارج از محدوده نرمال است";
      }
 ...
 // تشخیص حاد یا مزمن بودن اختلال تنفسی
const determineChronicity = (pH, pCO2) => {
    const pCO2Num = parseFloat(pCO2);
    
    // فرمول Winter برای جبران متابولیک در اسیدوز تنفسی
    if (pCO2Num > 45) { // اسیدوز تنفسی
      const expectedHCO3 = 24 + 0.1 * (pCO2Num - 40); // افزایش 0.1 mEq/L به ازای هر 1 mmHg pCO2 برای هر ساعت
      const actualHCO3 = parseFloat(abgValues.HCO3);
      
      if (Math.abs(actualHCO3 - expectedHCO3) <= 2) {
        return "مزمن";
      } else if (actualHCO3 < expectedHCO3) {
        return "حاد";
      } else {
        return "مختلط";
      }
    ...
    // فرمول برای آلکالوز تنفسی
    if (pCO2Num < 35) {
      const expectedHCO3 = 24 - 0.2 * (40 - pCO2Num); // کاهش 0.2 mEq/L به ازای هر 1 mmHg pCO2
      const actualHCO3 = parseFloat(abgValues.HCO3);
  ...
  // تفسیر دقیق ABG برای کودکان بر اساس رفرنس‌های پزشکی
  const interpretABG = () => {
    if (!validateABG()) {
      return;
    }
    ...
    if (pHNum < 7.35) {
      // اسیدمی
      if (pCO2Num > 45) {
        interpretation.primary = "اسیدوز تنفسی";
        const chronicity = determineChronicity(pH, pCO2);
        interpretation.chronicity = chronicity;
        ...
        interpretation.primary = "اسیدوز متابولیک";
        
      
        
        // جبران تنفسی مورد انتظار (فرمول Winter)
        const expectedPCO2 = (1.5 * HCO3Num) + 8;
        if (Math.abs(pCO2Num - expectedPCO2) <= 2) {
          interpretation.compensation = "جبران تنفسی مناسب";
        } else if (pCO2Num > expectedPCO2) {
          interpretation.compensation = "اسیدوز تنفسی اضافی";
        } else {
          interpretation.compensation = "آلکالوز تنفسی اضافی";
        }
      ...
      // آلکالمی
      if (pCO2Num < 35) {
        interpretation.primary = "آلکالوز تنفسی";
        const chronicity = determineChronicity(pH, pCO2);
        interpretation.chronicity = chronicity;
        ...
        interpretation.primary = "آلکالوز متابولیک";
      ...
      interpretation.primary = "ABG نرمال";
    }

    // تشخیص اختلالات مختلط
    if (pHNum >= 7.35 && pHNum <= 7.45) {
      if (pCO2Num > 45 && HCO3Num > 26) {
        interpretation.primary = " آلکالوز متابولیک جبران شده با اسیدوز تنفسی";
      } else if (pCO2Num < 35 && HCO3Num < 22) {
        interpretation.primary = " اسیدوز متابولیک جبران شده با آلکالوز تنفسی";
      }
    }

   // ارزیابی اکسیژناسیون   
if (pO2Num < 60) {
  interpretation.oxygenation = "هیپوکسمی شدید";
  newSettings.fio2 = Math.min(100, parseInt(currentSettings.fio2) + 30);
  newSettings.peep = Math.min(12, parseInt(currentSettings.peep) + 3);
} else if (pO2Num < 80) {
  interpretation.oxygenation = "هیپوکسمی";
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <div className="space-y-2 text-blue-700">
              <p className="font-semibold text-lg"> {abgInterpretation.primary}{abgInterpretation.secondary}</p>
              {abgInterpretation.chronicity && (
                <p className="font-medium"> {abgInterpretation.chronicity}</p>
              )}
              {abgInterpretation.compensation && (
                <p className="font-medium"> {abgInterpretation.compensation}</p>
              )}
              {abgInterpretation.oxygenation && (
                <p className="font-medium"> {abgInterpretation.oxygenation}</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
            ...
            <div className="text-purple-700 text-sm space-y-2 text-right">
              <p><strong>فرمول‌های جبرانی:</strong></p>
              <p>• <strong>اسیدوز متابولیک</strong>: pCO₂ مورد انتظار = (1.5 × HCO₃) + 8 ± 2</p>
              <p>• <strong>آلکالوز متابولیک</strong>: pCO₂ مورد انتظار = (0.7 × HCO₃) + 20 ± 3</p>
              <p>• <strong>اسیدوز تنفسی حاد</strong>: HCO₃ افزایش 0.1 mEq/L به ازای هر mmHg pCO₂</p>
              <p>• <strong>اسیدوز تنفسی مزمن</strong>: HCO₃ افزایش 0.3-0.4 mEq/L به ازای هر mmHg pCO₂</p>
